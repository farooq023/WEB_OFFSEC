...
const Payloads = ({ setAlert, auth: { user } }) => {
  ...
  const onSubmit = (e) => {
    ...
    if (!filter.test(domain)){
      ...
      fetch("/api/sendgen/"+user.email+'/'+domain, {
        method: "POST",
      })
  ...
  return (
    <div style={{height:"100vh", width:"100%", backgroundColor:"#F0F2F5", display:"flex", alignItems:"center", flexDirection:"column"}}>      
      ...
      <h4 style={{color:"#17a2b8"}}><u>Selected Technique</u>: Generate Payloads</h4>
...
export default connect(mapStateToProps,{ setAlert})(Payloads);
